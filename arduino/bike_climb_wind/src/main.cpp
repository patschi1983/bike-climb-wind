...
uint16_t liftDownEndStop = 0;
uint16_t liftUpEndStop = 0;
uint16_t liftLevelPos = 0;
uint16_t liftTargetDistance = 0;     // the target distance
uint16_t liftLastTargetDistance = 0; // the last target distance

#define LIFT_MODE_SETUP 1
#define LIFT_MODE_LEVEL 2
#define LIFT_MODE_MANUAL 3
#define LIFT_MODE_AUTO 4

int liftMode = LIFT_MODE_SETUP; // setup on startup
...
void liftFillDistanceMovingAverage(int fillFactor);
void liftCalcTargetGrade();
void liftUp();
void liftDown();
void liftStop();
void liftSetupEndstopsAndLevelPos();
void liftLevel();
void liftSetMode(int liftMode);
bool liftIsAutoModeSet();
...
void oledLiftSetupDisplay(int seconds);
...
void loop()
{
    liftSetupEndstopsAndLevelPos(); // initially measure end stops
        ...
        { // File exists, reading and loading
            ...
            if (configFile)
            {
                ...
                else
                {
                    ...
                    // Read lift configuration
                    if (!configJson["liftUpEndStop"].isNull())
                    {
                        liftUpEndStop = configJson["liftUpEndStop"];
                    }
                    if (!configJson["liftDownEndStop"].isNull())
                    {
                        liftDownEndStop = configJson["liftDownEndStop"];
                    }
                    if (!configJson["liftLevelPos"].isNull())
                    {
...
void liftSetup()
{
    ...
    liftFillDistanceMovingAverage(1);
...
void liftControl()
{
    ...
    debugPrintln(String(F("Calculated and set target distance - ")) + String(liftTargetDistance));
    debugPrintln(String(F("Last set target distance - ")) + String(liftLastTargetDistance));

    // limit min distance
    if (liftTargetDistance < liftUpEndStop)
    {
        liftTargetDistance = liftUpEndStop;
        debugPrintln(String(F("Lift 'UP' endstop reached - limited to")) + String(liftUpEndStop));
    }

    // limit max distance
    if (liftTargetDistance > liftDownEndStop)
    {
        liftTargetDistance = liftDownEndStop;
        debugPrintln(String(F("Lift 'DOWN' endstop reached - limited to")) + String(liftDownEndStop));
    }

    // check if lift action is needed
    if (!liftIsMoving() && (liftTargetDistance == liftLastTargetDistance))
    {
...
void liftFillDistanceMovingAverage(int fillFactor)
{
    ...
    for (int i = 0; i < (liftDistanceMvgAvgLen * fillFactor); i++)
    {
        liftReadDistance();
        sleep(1);
    }
...
void liftCalcTargetGrade()
{
    if (!liftIsAutoModeSet)
    {
        // nothing to do beacuse mode is not automatic
        return;
    }
...
void liftSetupEndstopsAndLevelPos()
{
    if (liftMode == LIFT_MODE_SETUP)
    {
        if (liftUpEndStop <= 0 && liftDownEndStop <= 0 && liftLevelPos <= 0) // only setup if no data is present
        {
            oledLiftSetupDisplay(0);

            // starting to determine upper/up endstop
            liftUp();
            sleep(20);
            liftFillDistanceMovingAverage(3);
            liftUpEndStop = (liftDistance + 9) / 10 * 10; // round to next higher 10
            oledLiftSetupDisplay(0);

            // starting to determine lower/down endstop
            liftDown();
            sleep(20);
            liftFillDistanceMovingAverage(3);
            liftDownEndStop = (liftDistance - 9) / 10 * 10; // round to next lower 10
            oledLiftSetupDisplay(0);

            // calculate level pos 20% (2/3) up 10% (1/3) down for 300mm actucator
            liftLevelPos = liftUpEndStop + 200;
            oledLiftSetupDisplay(0);
...
void liftSetMode(int liftMode2Set)
{
    liftMode = liftMode2Set;
}

bool liftIsAutoModeSet()
{
    return (liftMode == LIFT_MODE_AUTO);
}
...
void oledLiftSetupDisplay(int seconds)
{
    ...
    oledPrintTextCenter(String(F("CLIMB & BLOW")));
    oledDisplay.println();
    oledPrintTextCenter(String(F("Lift calibration...")));
    oledDisplay.println();
    oledPrintTextCenter(String(F("UP# ")) + String(liftUpEndStop) + String(F(" - DOWN# ")) + String(liftDownEndStop));
    oledPrintTextCenter(String(F("LEVEL# ")) + String(liftLevelPos));
    oledDisplay.display();
}
...
void oledPrintInclineLine()
{
    ...
    case 1:
        ...
        break;
    case 0:
        oledPrintTextRight(liftIsAutoModeSet() ? String(F("AUT")) : String(F("MAN")));
        break;
    case -1:
